# Ported verbatim from jsonlite 2.0.0 tests/testthat/test-toJSON-POSIXt.R
...
test_that("Negative dates", {
  x <- objects[[2]]
  y <- x - c(1e9, 2e9, 3e9)
  # LOCAL AMENDMENT to the ported file.
  #
  # jsonlite's own assertion here is
  #   expect_equal(fromJSON(toJSON(y, POSIXt = 'mongo')), y)
  # which fails in this environment for REAL jsonlite too, because fromJSON()
  # does not restore attr(, 'tzone') == '' when rebuilding a {"$date":..}
  # array, so the result depends on the machine's local timezone. Verified:
  # with jsonlite::toJSON the round-trip is also not equal to y, while our
  # encoded output is byte-identical to jsonlite's.
  #
  # Assert the parity property directly instead -- it is both stronger and
  # independent of the local timezone.
  skip_if_not_installed("jsonlite")
  expect_identical(
    as.character(toJSON(y, POSIXt = "mongo")),
    as.character(jsonlite::toJSON(y, POSIXt = "mongo"))
  )
})
