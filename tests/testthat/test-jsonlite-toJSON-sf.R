# Ported verbatim from jsonlite 2.0.0 tests/testthat/test-toJSON-sf.R
...
#
# This file's helper calls toJSON(sf_obj) with no sf= argument and asserts the
# record-array shape, because jsonlite 2.0.0 defaults sf objects to
# sf = "dataframe". fastgeojson defaults to sf = "geojson", since emitting a
# FeatureCollection for an sf object is the purpose of the package. Rather
# than skip the file -- it is our only check against GDAL's own writer -- opt
# into jsonlite's default for its duration, which is exactly what
# options(fastgeojson.sf=) exists for. All three modes are byte-identical to
# jsonlite when requested explicitly; only the default differs.
.fgj_old_sf_default <- options(fastgeojson.sf = "dataframe")
# restored at the foot of this file (testthat traps errors inside test_that,
# so the restore is always reached)
