...
`fastgeojson` provides extremely fast conversion of `sf` objects to GeoJSON FeatureCollections and generic R objects (`data.frame`, lists, vectors) to JSON strings.
...
> **Status: v0.2.0** — Introduces the "omnivore" `as_json()` function and architectural optimizations (Arena memory layout, Direct-Heap writing). 
...
The `as_json()` function is designed to replicate the behavior of `jsonlite::toJSON()` in virtually all edge cases, acting as a high-fidelity drop-in replacement.
...
- **`as_json(x)`**: The "omnivore" function. It automatically detects the input type (`sf` object, data frame, list, or vector) and dispatches it to the optimized Rust encoders.
...
Behaves identically to `jsonlite` for standard R structures, preserving types and hierarchies.
...
`fastgeojson` v0.2.0 supports serialization for a wide range of R data types, ensuring compatibility with `jsonlite`:
