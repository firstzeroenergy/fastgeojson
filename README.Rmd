...
Implemented in Rust via the **extendr** framework, it uses parallel processing and low-level optimizations to deliver **1.5–6× speedups** over existing R solutions on large datasets.
...
data.frame(
  ...
  Median_ms   = c(1575, 1348, 231, 158),
  `Speedup vs yyjsonr` = c("—", "—", "1×", "1.5×")
) |> 
  knitr::kable()
```
...
data.frame(
  ...
  Median_ms   = c(1940, 563, 332),
  `Speedup vs yyjsonr` = c("—", "1×", "1.7×")
) |> 
  knitr::kable()
```
