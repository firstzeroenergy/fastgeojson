...
> **Status: v0.1.1** — Stable for production use and compatible with `shinyapps.io`. This is an early release of `fastgeojson`, so we are actively looking for edge cases; bug reports and feature requests are very welcome.
...
To prioritize stability and speed, the following complex types are currently not supported. Future updates may introduce support for these types, provided they do not compromise encoding performance.

- List-Columns: Columns containing nested lists (e.g., `c("tag1", "tag2")` inside a cell) are omitted from the output.
  * *Workaround:* Flatten list columns into strings before conversion.
  ```r
  df$tags <- sapply(df$tags, paste, collapse = ", ")
  ```
- POSIXlt: The `POSIXlt` format is not supported due to its underlying list-based structure.
