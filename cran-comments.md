...
* Windows 11, R 4.5.1 (local), Rtools 4.5, rustc 1.75.0 — `devtools::check(cran = TRUE, remote = TRUE, incoming = TRUE)` and `R CMD check --as-cran`; the install, R test suite and Rust unit tests were repeated with rustc 1.68.0 (the declared minimum) and 1.92.0 (0 warnings)
...
One entry point outside the documented API is used deliberately: `ATTRIB`, in a single function (`attrib_by_tag()`, `src/rust/src/capi.rs`) that reads the `dim` attribute of a coordinate matrix from inside a worker thread. The documented substitute, `Rf_getAttrib`, calls `MARK_NOT_MUTABLE` on the attribute it returns — a write to a shared SEXP header — and this function runs concurrently across the geometries of one `sfc`, so several threads would write the same header at once. The alternative is to pre-read every geometry's `dim` serially on the R thread, which is the pass the parallel extraction exists to avoid. The read touches three pointers and writes nothing; it is documented in place. All other entry points used are in R's API or experimental API table (`ALTREP`, `Rf_charIsUTF8`, `Rf_charIsLatin1` are the experimental ones; verified with `tools:::checkLibAPI()` on the Linux build under R 4.5.1).
