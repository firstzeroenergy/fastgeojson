...
jsonlite's own `toJSON` test suite (jsonlite 2.0.0, in `tests/testthat/` with `toJSON()` bound to `as_json()`) passes with no failures and no skips, including its `sf` tests against GDAL's GeoJSON writer.
...
- Numbers: the fixed-decimal path ports `jsonlite`'s `modp_dtoa2`; `digits = Inf` uses Żmij, byte-identical to `ryu`.
